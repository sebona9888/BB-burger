...
const AdminDashboard = () => {
    ...
    const fetchOrders = async () => {
        try {
            const res = await axios.get('https://prime-backend.onrender.com/api/orders', {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editing) {
                await axios.put(`https://prime-backend.onrender.com/api/menu/${editing._id}`, burgerData, {
                    headers: { 'admin-secret': 'admin123' }
                });
                toast.success('Burger updated! 🍔', { id: loadingToast });
            } else {
                await axios.post('https://prime-backend.onrender.com/api/menu', burgerData, {
                    headers: { 'admin-secret': 'admin123' }
                });
    ...
    const deleteBurger = async (id) => {
        ...
        try {
            await axios.delete(`https://prime-backend.onrender.com/api/menu/${id}`, {
                headers: { 'admin-secret': 'admin123' }
            });
    ...
    const updateOrderStatus = async (id, status) => {
        try {
            await axios.put(`https://prime-backend.onrender.com/api/orders/${id}`, { status }, {
                headers: { 'admin-secret': 'admin123' }
            });
